...
public class GbfsIshtarManagement {
  ...
  public ResponseEntity<Map<String, Object>> deleteProvider(
    ...
    try {
      ...
      if (existing == null) {
        return ResponseEntity.ok(
          Map.of(
            ...
            true,
            "message",
            "The subscription has not been activated, so unsubscribing is not necessary."
          )
        );
      }
      ...
      return ResponseEntity.ok(
        Map.of("success", true, "message", "Unsubscribe successfully")
      );
  ...
  public ResponseEntity<Map<String, Object>> synchronize() {
    try {
      ...
      return ResponseEntity.ok(
        Map.of("success", true, "message", "Synchronized successfully")
      );
    } catch (Exception e) {
  ...
  private boolean hasChanged(FeedProvider existing, FeedProvider newProvider) {
    ...
    boolean basicFieldsChanged =
      !Objects.equals(existing.getOperatorId(), newProvider.getOperatorId()) ||
    ...
    boolean collectionsChanged =
      !listsEqual(existing.getVehicleTypes(), newProvider.getVehicleTypes()) ||
    ...
    return (
      !Objects.equals(
        existingAuth.getRequestAuthenticator(),
        newAuth.getRequestAuthenticator()
      ) ||
  ...
  private boolean propertiesEqual(
