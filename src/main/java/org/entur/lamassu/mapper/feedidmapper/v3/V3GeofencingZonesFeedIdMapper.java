...
@Component
public class V3GeofencingZonesFeedIdMapper implements FeedIdMapper<GBFSGeofencingZones> {

  private final IdMappingService idMappingService;

  public V3GeofencingZonesFeedIdMapper(IdMappingService idMappingService) {
  ...
  public void mapIds(
    ...
    Set<String> vehicleTypeIds = rules
      .stream()
      .map(GBFSRule::getVehicleTypeIds)
    ...
    for (var rule : rules) {
      rule.setVehicleTypeIds(
        IdMappersUtil.mapIds(rule.getVehicleTypeIds(), vehicleTypeIdsMap)
      );
    }
  }
}
